...
class Network:
    def __init__(self, Topo, Train, Test, learn_rate, prob_type):
        ...
        self.B1 = np.random.randn(1, self.Top[1]) / np.sqrt(
            self.Top[1]
        )  # bias first layer
        self.W2 = np.random.randn(self.Top[1], self.Top[2]) / np.sqrt(self.Top[1])
        self.B2 = np.random.randn(1, self.Top[2]) / np.sqrt(
            self.Top[1]
        )  # bias second layer
    ...
    def BackwardPass(self, Input, desired):
        ...
        if (
            self.prob == "classification"
        ):  # ensure that one-hot encoding is used in data - for classification problems
    ...
    def decode(self, w):
        ...
        self.B1 = w[
            w_layer1size + w_layer2size : w_layer1size + w_layer2size + self.Top[1]
        ]
        self.B2 = w[
            w_layer1size
            + w_layer2size
            + self.Top[1] : w_layer1size
            + w_layer2size
            + self.Top[1]
            + self.Top[2]
        ]
    ...
    def evaluate_proposal(self, data, w):  # BP with SGD (Stocastic BP)
        ...
        for i in range(
            0, size
        ):  # to see what fx is produced by your current weight update
            Input[:] = data[i, 0 : self.Top[0]]
            Desired[:] = data[i, self.Top[0] :]
            self.ForwardPass(Input)
...
class MCMC:
    ...
    def rmse(self, predictions, targets):
        return np.sqrt(
            np.sum(np.square(np.subtract(predictions, targets))) / targets.shape[0]
        )
    ...
    def loglikelihood(self, neuralnet, data, w, tausq):
        ...
        if self.prob == "regression":
            [log_lhood, prediction, perf] = self.gaussian_loglikelihood(
                neuralnet, data, w, tausq
            )
        elif self.prob == "classification":
            [log_lhood, prediction, perf] = self.multinomial_loglikelihood(
                neuralnet, data, w
            )
    ...
    def gaussian_loglikelihood(self, neuralnet, data, w, tausq):
        ...
        log_lhood = -n / 2 * np.log(2 * math.pi * tausq) - (1 / (2 * tausq)) * np.sum(
            np.square(y - fx)
        )
        return [log_lhood, fx, rmse]

    def prior_regression(
        self, sigma_squared, nu_1, nu_2, w, tausq
    ):  # for weights and biases and tausq
    ...
    def prior_classification(
        self, sigma_squared, nu_1, nu_2, w
    ):  # for weights and biases only
        h = self.topology[1]  # number hidden neurons
        d = self.topology[0]  # number input neurons
        part1 = (
            -1
            * ((d * h + h + self.topology[2] + h * self.topology[2]) / 2)
    ...
    def sampler(self, w_limit, tau_limit):
        ...
        w_size = (
            (netw[0] * netw[1]) + (netw[1] * netw[2]) + netw[1] + netw[2]
        ...
        fxtrain_samples = np.ones(
            (samples, trainsize, netw[2])
        )  # fx of train data over all samples
        fxtest_samples = np.ones(
            (samples, testsize, netw[2])
        )  # fx of test data over all samples
        ...
        neuralnet = Network(
            self.topology, self.traindata, self.testdata, self.learn_rate, self.prob
        )
        print("evaluate Initial w")

        pred_train, prob = neuralnet.evaluate_proposal(
            self.traindata, w
        )  # ignore prob in case of regression problems
        ...
        prior_current = self.prior(
            sigma_squared, nu_1, nu_2, w, tau_pro
        )  # takes care of the gradients

        [likelihood, pred_train, p_train] = self.loglikelihood(
            neuralnet, self.traindata, w, tau_pro
        )
        [likelihood_ignore, pred_test, p_test] = self.loglikelihood(
            neuralnet, self.testdata, w, tau_pro
        )
        ...
        for i in range(samples - 1):
            ...
            if (self.use_langevin_gradients is True) and (lx < self.l_prob):
                w_gd = neuralnet.langevin_gradient(
                    self.traindata, w.copy(), self.sgd_depth
                )
                w_proposal = np.random.normal(w_gd, step_w, w_size)
                w_prop_gd = neuralnet.langevin_gradient(
                    self.traindata, w_proposal.copy(), self.sgd_depth
                )
                ...
                first = (
                    -0.5 * np.sum(wc_delta * wc_delta) / sigma_sq
            ...
            [likelihood_proposal, pred_train, p_train] = self.loglikelihood(
                neuralnet, self.traindata, w_proposal, tau_pro
            )
            [likelihood_ignore, pred_test, p_test] = self.loglikelihood(
                neuralnet, self.testdata, w_proposal, tau_pro
            )

            prior_prop = self.prior(
            ...
            if u < mh_prob:
                ...
                pos_tau[
                    i + 1,
                ] = tau_pro  # only used for regression
                fxtrain_samples[
                    i + 1,
                ] = pred_train
                fxtest_samples[
                ...
                fxtrain_samples[i + 1,] = fxtrain_samples[
                    i,
                ]
                fxtest_samples[i + 1,] = fxtest_samples[
                    i,
                ]
                perf_train[i + 1,] = perf_train[
                    i,
                ]
                perf_test[i + 1,] = perf_test[
                    i,
                ]
...
def scikit_linear_mod(
    x_train, x_test, y_train, y_test
):  # in case we want to compare with other models in future
...
def main():
    for problem in range(1, 6):
        ...
        if problem == 3:
            ...
            data_inputy = (
                data_in[:, 8] / 50
            )  # this is target - so that last col is selectedd from data  - we normlise the targe [0,1]
            percent_test = 0.4
            x_train, x_test, y_train, y_test = train_test_split(
                data_inputx, data_inputy, test_size=percent_test, random_state=0
            )

            traindata = np.column_stack(
                (x_train, y_train)
            )  # we need to do this since our code takes data in this format
        ...
        if problem == 4:
            ...
            traindata = np.hstack(
                [
                    features[indices[: np.int(train_ratio * features.shape[0])], :],
        ...
        if problem == 5:
            traindata = np.genfromtxt("data/ionesphere/ftrain.csv", delimiter=",")[
                :, :-1
            ]
        ...
        [
            ...
            p_test,
            accept_ratio,
        ] = mcmc.sampler(w_limit, tau_limit)
