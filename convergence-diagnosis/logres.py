...
class lin_model:
    def __init__(
        self, num_epocs, train_data, test_data, num_features, learn_rate, activation
    ):
        ...
        self.use_activation = (
            activation  # SIGMOID # 1 is  sigmoid , 2 is step, 3 is linear
        )
    ...
    def squared_error(self, prediction, actual):
        ...
        )  # to cater more in one output/class
...
class MCMC:
    ...
    def sampler(self):
        ...
        )  # num of weights and bias (eg. 4 + 1 in case of time series problems used)
        ...
        step_w = 0.02
        # defines how much variation you need in changes to w
        step_eta = 0.01
        ...
        model = lin_model(
            0, self.traindata, self.testdata, self.topology[0], 0.1, self.regression
        )
        ...
        for i in range(samples - 1):
            ...
            [likelihood_proposal, pred_train, rmsetrain] = self.likelihood_func(
                model, self.traindata, w_proposal, tau_pro
            )
            [likelihood_ignore, pred_test, rmsetest] = self.likelihood_func(
                model, self.testdata, w_proposal, tau_pro
            )
            ...
            if u < mh_prob:
                ...
                rmse_train[i + 1,] = rmse_train[
                    i,
                ]
                rmse_test[i + 1,] = rmse_test[
                    i,
                ]
        ...
        print(
            ...
            rmse_tes,
            rmsetest_std,
            " rmse_tr, rmsetr_std, rmse_tes, rmsetest_std",
        )
        ...
        for i in range(num_trials):
            ...
            tausq_drawn = np.random.normal(
                pos_tau.mean(), pos_tau.std()
            )  # a buf is present here - gives negative values at times

            [loss, fx_, accuracy[i]] = self.likelihood_func(
                model, self.testdata, w_drawn, tausq_drawn
            )
...
def main():
    ...
    for problem in range(2, 3):
        ...
        mcmc = MCMC(
            numSamples, traindata, testdata, topology, activation
        )  # declare class
        ...
        np.savetxt(
            outres,
            (rmse_tr, rmsetr_std, rmse_tes, rmsetest_std, accept_ratio),
            fmt="%1.5f",
        )
